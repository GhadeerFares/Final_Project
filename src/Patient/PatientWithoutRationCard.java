 ...
 * @author Ghadeer0
 */
public class PatientWithoutRationCard extends Patient{
    ...
    public void cost(int id, float price) throws Exception {
         ...
         float cost = (float) (price +price*30/150) ;
         
           DataConnection aDataConnection = DataConnection.getDataConnection();
        
            aStatement = aDataConnection.getStatement();
            
            aStatement.execute("insert into patientwithoutrationcard(id,price,cost) values ('" + id+ "','"  + price + "','" + cost + "')");
